...
class ReaderViewModel @JvmOverloads constructor(
    ...
    fun openPageDialog(page: ReaderPage, position: ManualTranslationPosition?) {
        mutableState.update { it.copy(dialog = Dialog.PageActions(page, position)) }
    }

    fun addManualTranslation(
        sourceText: String,
        translation: String,
        ...
        val position = dialog?.translationPosition
        val editor = page?.translationEditor
        if (page == null || position == null || editor == null) {
            onResult(Result.failure(IllegalStateException("Não foi possível localizar este balão na página.")))
            return
        }

        editor.addManualTranslation(position, sourceText, translation) { result ->
            result.onSuccess { added ->
                val pageTranslation = page.translation ?: PageTranslation(
                    imgWidth = added.pageWidth,
                    imgHeight = added.pageHeight,
                ).also { page.translation = it }
                pageTranslation.imgWidth = added.pageWidth
                pageTranslation.imgHeight = added.pageHeight
                pageTranslation.blocks.add(
                    index = added.blockIndex.coerceIn(0, pageTranslation.blocks.size),
                    element = added.block,
                )
                // Let Compose close both dialogs before notifying the attached viewer.
                // Rebuilding the overlay while the dialog hierarchy was being dismissed
                // could tear down the reader activity on some devices.
                onResult(Result.success(Unit))
                viewModelScope.launch { eventChannel.send(Event.RefreshTranslations(page)) }
            }.onFailure { error ->
    ...
    sealed interface Dialog {
        ...
        data class PageActions(
            val page: ReaderPage,
            val translationPosition: ManualTranslationPosition?,
        ) : Dialog
    }

    sealed interface Event {
        data object ReloadViewerChapters : Event
        data class RefreshTranslations(val page: ReaderPage) : Event
