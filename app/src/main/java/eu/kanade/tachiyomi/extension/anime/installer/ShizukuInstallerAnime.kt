...
class ShizukuInstallerAnime(private val service: Service) : InstallerAnime(service) {
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (status == PackageInstaller.STATUS_SUCCESS) {
                ...
                logcat(LogPriority.ERROR) { "Failed to install anime extension $packageName: $message" }
    ...
    override fun processEntry(entry: Entry) {
        ...
        try {
            ...
            logcat(LogPriority.ERROR, e) { "Failed to install anime extension ${entry.downloadId} ${entry.uri}" }
            continueQueue(InstallStep.Error)
        }
    }
