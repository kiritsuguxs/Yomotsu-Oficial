...
internal fun ColumnScope.ReadingModePage(viewModel: ReaderSettingsViewModel) {
    ...
    CheckboxItem(
        ...
        pref = viewModel.preferences.showTranslations,
    )
...
private fun ColumnScope.WebtoonViewerSettings(viewModel: ReaderSettingsViewModel) {
    ...
    val displayedAutoScrollSpeed = autoScrollSpeed.coerceIn(
        ReaderPreferences.AUTO_SCROLL_SPEED_MIN,
        ReaderPreferences.AUTO_SCROLL_SPEED_MAX,
    )
    SliderItem(
        value = displayedAutoScrollSpeed,
        valueRange = ReaderPreferences.AUTO_SCROLL_SPEED_MIN..ReaderPreferences.AUTO_SCROLL_SPEED_MAX step
            ReaderPreferences.AUTO_SCROLL_SPEED_STEP,
        steps = 15,
        label = stringResource(MR.strings.pref_auto_scroll_speed),
        valueString = stringResource(MR.strings.auto_scroll_speed_value, displayedAutoScrollSpeed),
        onChange = viewModel.preferences.autoScrollSpeed::set,
        pillColor = MaterialTheme.colorScheme.surfaceContainerHighest,
    )
