...
private fun ColumnScope.FilterPage(
    ...
    val animeFilterUnseen by screenModel.libraryPreferences.animeFilterUnseen.collectAsState()
    TriStateItem(
        ...
        onClick = { screenModel.toggleFilter(LibraryPreferences::animeFilterUnseen) },
    )
    val animeFilterStarted by screenModel.libraryPreferences.animeFilterStarted.collectAsState()
    TriStateItem(
        label = stringResource(MR.strings.label_started),
        state = animeFilterStarted,
        onClick = { screenModel.toggleFilter(LibraryPreferences::animeFilterStarted) },
    )
    val animeFilterBookmarked by screenModel.libraryPreferences.animeFilterBookmarked.collectAsState()
    TriStateItem(
        label = stringResource(MR.strings.action_filter_bookmarked),
        state = animeFilterBookmarked,
        onClick = { screenModel.toggleFilter(LibraryPreferences::animeFilterBookmarked) },
    )
    val animeFilterCompleted by screenModel.libraryPreferences.animeFilterCompleted.collectAsState()
    TriStateItem(
        label = stringResource(MR.strings.completed),
        state = animeFilterCompleted,
        onClick = { screenModel.toggleFilter(LibraryPreferences::animeFilterCompleted) },
    )
    ...
    when (trackers.size) {
        ...
        1 -> {
            ...
            val filterTracker by screenModel.libraryPreferences.filterTrackedAnime(service.id.toInt())
                .collectAsState()
            TriStateItem(
...
private fun ColumnScope.SortPage(
    ...
    val trackers by screenModel.trackersFlow.collectAsState()
    val sort = screenModel.getSortFor(category)
